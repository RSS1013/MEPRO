...
public class CartaTextoColor {
	...
	/** Indicador de ejecución en Eclipse. */
	 ...
	 */
	private static Attribute obtenerColorPalo(final Palo palo) {
	 ...
	 */
	private static String puntuacionAValorVisual(final int puntuacion) {
	 ...
	 */
	private static String obtenerNombrePalo(final Palo palo) {
		final String nombre = palo.toString().toLowerCase();
	 ...
	 */
	public static String[] dibujarCarta(final Carta carta) {
		final String valor = puntuacionAValorVisual(carta.puntuacion());
		final String nombrePalo = obtenerNombrePalo(carta.palo());
		final Attribute colorFondo = obtenerColorPalo(carta.palo());
	 ...
	 */
	private static String[] dibujarCarta(final String valor, final String nombrePalo, final Attribute colorFondo) {
		final int desp = valor.length() > 1 ? valor.length() : 2;
		final String filaArriba = String.format("│%-2s%-" + (ANCHO - desp) + "s│", valor, "");
		final String filaAbajo = String.format("│%" + (ANCHO - desp) + "s%2s│", "", valor);

		// Calcular centrado del texto del palo
		final int longitud = nombrePalo.length();
		final int espacios = ANCHO - longitud;
		final int izq = espacios / 2;
		final int der = espacios - izq;
		final String paloCentrado = " ".repeat(izq) + nombrePalo + " ".repeat(der);

		final String filaCentral;
	...
	public static boolean estaEjecutandoseDesdeEclipse() {
		...
		while (seguir) {
			final Optional<ProcessHandle> parent = handle.parent();
			if (parent.isEmpty()) {
				seguir = false;
			} else {
				handle = parent.get();
				final Optional<String> cmd = handle.info().command();
				if (cmd.isPresent()) {
					final String lower = cmd.get().toLowerCase();
	...
	private static boolean soportaAnsi() {
		final String sistemaOperativo = System.getProperty("os.name").toLowerCase();
